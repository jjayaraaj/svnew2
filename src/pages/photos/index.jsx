...
import { getAllPhotos } from '@/lib/getAllArticles'
  ...
  return (
    <article className="md:grid md:grid-cols-4 md:items-baseline">
      <Card className="md:col-span-3">
        <Card.Title href={`/photos/${article.slug}`}>
          {article.title}
        </Card.Title>
        <Card.Eyebrow
          as="time"
...
export default function ArticlesIndex({ articles }) {
  return (
    <>
      <Head>
        <title>Photos - Sathyan Velumani</title>
        <meta
          name="description"
      ...
      <SimpleLayout
        title="Some notes on the photos"
...
export async function getStaticProps() {
  return {
    props: {
      articles: (await getAllPhotos()).map(({ component, ...meta }) => meta),
    },
  }
}
