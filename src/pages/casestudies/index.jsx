...
import { getAllCasestudies } from '@/lib/getAllArticles'
  ...
  return (
    <article className="md:grid md:grid-cols-4 md:items-baseline">
      <Card className="md:col-span-3">
        <Card.Title href={`/casestudies/${article.slug}`}>
          {article.title}
        </Card.Title>
        <Card.Eyebrow
          as="time"
...
export default function ArticlesIndex({ articles }) {
  return (
    <>
      <Head>
        <title>Articles - Sathyan Velumani</title>
        <meta
          name="description"
...
export async function getStaticProps() {
  return {
    props: {
      articles: (await getAllCasestudies()).map(({ component, ...meta }) => meta),
    },
  }
}
